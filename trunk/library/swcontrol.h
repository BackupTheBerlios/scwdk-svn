...
class swcontrol: public mutex, public swthread{
public:
   typedef sigc::signal<bool> TWorkDelegator;
   typedef sigc::slot<bool> TWorkDelegate;
   swcontrol();
   swcontrol(const std::string& nameid);
   virtual ~swcontrol();
   int wait();
   void signal();

   TWorkDelegator::iterator delegate( swcontrol::TWorkDelegate& _work ) { return proceed_.connect(_work); }
   virtual bool start();
   virtual int init();
